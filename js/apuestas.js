(function() {
  ...
  function renderMatches() {
    ...
    jornada.forEach(match => {
      ...
      let html = `<div class="match-header"><span>${home.name}</span><span style="color:var(--muted);font-size:0.9rem">vs</span><span>${away.name}</span></div>`;

      if (existingBet) {
        html += `<div style="text-align:center;padding:10px;background:rgba(34,197,94,0.1);border-radius:8px;border:1px solid var(--accent);">Apuesta Activa: <b>${existingBet.amount}€</b> al <b>${existingBet.type}</b></div>`;
      } else {
        html += `<div class="odds-container">
            <div class="odd-btn" onclick="selectOdd(this, '${matchKey}', '1', ${odds['1']})"><span class="odd-label">LOCAL</span><span class="odd-val">${odds['1']}</span></div>
            <div class="odd-btn" onclick="selectOdd(this, '${matchKey}', 'X', ${odds['X']})"><span class="odd-label">EMPATE</span><span class="odd-val">${odds['X']}</span></div>
            <div class="odd-btn" onclick="selectOdd(this, '${matchKey}', '2', ${odds['2']})"><span class="odd-label">VISITA</span><span class="odd-val">${odds['2']}</span></div>
          </div>
          <div class="bet-actions" id="actions-${matchKey}" style="display:none;">
            <input type="number" class="bet-input" id="input-${matchKey}" placeholder="€" min="1">
            <button class="btn-small" onclick="placeBet('${matchKey}')" style="background:var(--success);color:white;flex:1;">Apostar</button>
          </div>`;
      }
      card.innerHTML = html;
  ...
  window.selectOdd = function(el, matchKey, type, odds) {
    el.parentElement.querySelectorAll('.odd-btn').forEach(b => b.classList.remove('selected'));
    el.classList.add('selected');
    document.querySelectorAll('.bet-actions').forEach(a => a.style.display = 'none');
